...
class Agent:
    def __init__(self,fluid, pos, dir, movement_angle, color, speed):
        ...
        self.prev_movement_angle = movement_angle
        self.angle_dir = 1
        self.new_movement_angle_set = False
        self.counter = 0
        self.speed = speed

    def update(self, scale_vel, scale_dens, dt):
        self.angle += self.speed * dt * self.angle_dir
        ...
        if self.angle < self.original_angle and not self.new_movement_angle_set:
            self.movement_angle = random.uniform(math.pi/4, math.pi/2)
            self.speed = random.uniform(0.4, 0.8)
            self.new_movement_angle_set = True

        if abs(math.atan2(self.dir.y, self.dir.x) - self.original_angle) > self.movement_angle:
            self.counter += 1
            self.angle_dir *= -1
            if self.angle_dir == -1:
                self.angle = self.original_angle + self.movement_angle
                self.new_movement_angle_set = False
